...
async def handler(
    ...
    await call.message.edit_text(f"Введите фамилию для аккаунта - {commentator}.")
    call.bot["user_settings"][str(call.from_user.id)] = f'{commentator}|{phone}'
    await state.set_state(ChangeSurname.surname.state)
...
async def handler_2(
    ...
    try:
        ...
        await message.answer(f"Для комментатора {commentator} было установлена фамилия {surname}",
