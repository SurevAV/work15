...
async def handler(
    ...
    await call.message.edit_text(f"Введите username для аккаунта - {commentator}.")
    call.bot["user_settings"][str(call.from_user.id)] = f'{commentator}|{phone}'
    await state.set_state(ChangeUserName.username.state)
...
async def handler_2(
    ...
    try:
        ...
        async with db() as session:
            await session.execute(update(Commentator).values({Commentator.name: username}).where(
                Commentator.phone == phone))
            await session.commit()

        await message.answer(f"Для комментатора {commentator} было установлено username {username}",
                             reply_markup=make_keyboard([],
                                                        f'{main_menu_my_commentators_commentator_settings_settings_account.ID}|{username}|{phone}'))
    except Exception as e:
        await message.answer(f"{str(e)}", reply_markup=make_keyboard([],
